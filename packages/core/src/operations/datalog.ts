...
export const DatalogQuerySchema = z.object({
  ...
  inputs: z.array(z.unknown()).optional().describe("Input parameters for the query, corresponding to :in clause bindings after $"),
});

export type DatalogQueryParams = z.infer<typeof DatalogQuerySchema>;

export async function datalogQuery(client: RoamClient, params: DatalogQueryParams): Promise<CallToolResult> {
  const args = params.inputs ? [params.query, ...params.inputs] : [params.query];
  const response = await client.call<unknown[]>("q", args);
  const results = response.result ?? [];
  return textResult({ total: Array.isArray(results) ? results.length : 0, results });
}

...
- API versioning: `EXPECTED_API_VERSION` in `types.ts` is sent as `expectedApiVersion` in every request body (`client.ts`). Roam checks that major.minor match exactly — **patch is ignored**. So `1.1.0` and `1.1.2` are compatible, but `1.1.x` and `1.2.x` are not. On mismatch, `handleVersionMismatch()` in `client.ts` compares server vs expected versions and throws a `RoamError` with advice on which side to update. This is completely independent of the npm package version (`0.x.y`).
